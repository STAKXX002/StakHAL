...
pub fn build_state_diagram_panel() -> StateDiagramPanelWidgets {
    ...
    let combo_peripheral = gtk4::DropDown::from_strings(&["ALL PERIPHERALS"]);
    combo_peripheral.set_cursor_from_name(Some("pointer"));
    combo_peripheral.set_tooltip_text(Some("Filter by peripheral state machine"));

    let lbl_diagram_hint = gtk4::Label::builder()
        .label("Click node to inspect state transitions, drag to move")
    ...
    diagram_header_bar.append(&combo_peripheral);
    diagram_header_bar.append(&lbl_diagram_hint);

    let lbl_selected_info = gtk4::Label::builder()
        .label("Select a state node to view transition triggers and lifecycle details.")
        .halign(gtk4::Align::Start)
        .hexpand(true)
        .margin_start(18)
        .margin_end(18)
